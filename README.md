...
- **プレーンテキスト編集**：自動整形なし。Tab で2スペース挿入、Shift+Tab で戻す（複数行選択時は選択行をまとめてインデント・アンインデント）
- **優先タスクの抽出（今やるリスト）**：タスク行の先頭に `!` を付けると、上部のカードにプロジェクト名（直前の `#` 見出し）付きで優先度順に並ぶ
  - `- ! タスク` / `- !! タスク` / `- !!! タスク`。`!` の数が多いほど上に来る。同数なら本文の出現順
  - 本文中でも `!` 行は色が変わる（黄 / オレンジ / 赤）。カードの行をクリックすると本文のその行にジャンプ
- **URL のリンク表示**：本文中の http(s) URL に下線と色を付け、⌘+クリックで既定ブラウザで開く。テキスト自体は書き換えない
...
|---|---|
| `note.md` | メモ本文 |
| `note.bak.md` | 起動時に取られる前回分のバックアップ（1世代） |
